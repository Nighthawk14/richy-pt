jQuery(document).ready(function($){
    ...
    function calcTB_Pos() {
        $('#TB_window').css({
           ...
           'top' : 5+'%', // (($(window).height() + $(window).scrollTop())/2 - (($('#TB_ajaxContent').outerHeight()-$(window).scrollTop()) + 30)/2) + 'px',
           'opacity' : 1
        });
    }
  ...
  calcPercent();
    ...
    function dynamic_items(){
        ...
        var code = '';
        var tabID = '1', barID = '1', clientID = '1', columnID = '1', accordionID = '1', socialID='1', progressID='1';
        var tabContent, tabHeading, barPercent, tabColumn, tabAccordion, socialMedia, socialURL;
        var progressStyle, progressWidth, progressMin, progressMax, progressDefault, progressText
        var barTitle; 
        var clientImage, clientURL;

		//accordion
         if( $('.shortcode-options[data-name=ulysses_accordion]').is(':visible') ){
            $('.shortcode-options[data-name=ulysses_accordion] .shortcode-dynamic-item-text-title').each(function(){
              // if( $(this).val() != '' ) {
                    tabAccordion = $(this).parent().parent().find('.shortcode-dynamic-item-text-title').val();
                    tabContent = $(this).parent().parent().find('.shortcode-dynamic-item-text').val();
                    code += '[ulysses_toggle title="'+tabAccordion+'" id="accordion'+accordionID+'"]'+tabContent+'[/ulysses_toggle]'; 
                    accordionID++;
                //}
            });
         }		
        // column
        else if( $('.shortcode-options[data-name=ow_columns]').is(':visible') ){
            $('.shortcode-options[data-name=ow_columns] .shortcode-dynamic-item-size').each(function(){
              // if( $(this).val() != '' ) {
                    tabContent = $(this).parent().parent().find('.shortcode-dynamic-item-text').val();
                    tabHeading = $(this).parent().parent().find('.shortcode-dynamic-item-text-heading').val();
                    tabColumn = $(this).parent().parent().find('.shortcode-dynamic-item-size').val();
                    code += '[ow_column size="'+tabColumn+'" title="'+tabHeading+'" id="column'+columnID+'"]'+tabContent+'[/ow_column]'; 
                    columnID++;
                //}
            });
         }
                ...
                //social
         else if( $('.shortcode-options[data-name=ow_social]').is(':visible') ){
            $('.shortcode-options[data-name=ow_social] .shortcode-dynamic-item-url').each(function(){
              // if( $(this).val() != '' ) {
                    ...
                    code += '[ow_media name="'+socialMedia+'" url="'+socialURL+'"]'; 
                    socialID++;
                //}
            });
         }
                ...
                // progress
         else if( $('.shortcode-options[data-name=ow_progressbar]').is(':visible') ){
            $('.shortcode-options[data-name=ow_progressbar] .shortcode-dynamic-item-style').each(function(){
              // if( $(this).val() != '' ) {
                    ...
                    code += '[ow_bar style="'+progressStyle+'" width="'+progressWidth+'" min="'+progressMin+'" max="'+progressMax+'" default="'+progressDefault+'"] '+ progressText +' [/ow_bar]'; 
                    progressID++;
                //}
            });
         }
    ...
    function update_shortcode(ending){
        ...
        if(name == 'ow_icon' && $('.icon-option i.selected').length > 0 ) code += ' image="'+ $('.icon-option i.selected').attr('class').split(' ')[0] +'"'; 
    ...
    $('.shortcode-dynamic-item-input, .shortcode-dynamic-item-text, .shortcode-dynamic-item-text-title').live('keyup', function(){ dynamic_items(); });
