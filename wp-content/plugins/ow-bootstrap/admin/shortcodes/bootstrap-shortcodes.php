...
// Define Shortcodes
require_once( 'define-shortcode.php' );
...
<div id="bootstrap-shortcode-generator">
    <div class="shortcode-content">     
        <div class="label"><strong>Bootstrap Shortcodes</strong></div>            
        <div class="content">
			<select id="ow-bootstrap-shortcodes" data-placeholder="' . __("Choose a shortcode", 'ulysses') .'">
			<option value=""> - Select a shortcode - </option>';
			foreach( $ow_shortcodes as $shortcode => $options )
			{
				...
				else
				{
					...
					if( !empty($options['attr']) )
					{
						...
						{
							$html_options .= ow_option_element( $name, $attr_option, $options['type'], $shortcode );
						}
					}
					$html_options .= '</div>'; 
				}
...
<html>
<head>
	<link rel="stylesheet" href="<?php echo plugins_url('/css/shortcode_popup.css',__FILE__); ?>" />
	<script src="<?php echo plugins_url('/js/shortcode_popup.js',__FILE__); ?>"></script>
</head>
<body>
	<?php echo $shortcode_html . $html_options;  ?>
	<div id="shortcode-content">      
		<div class="label">
			<label id="option-label" for="shortcode-content">
				<strong><?php echo __( 'Content: ', 'ulysses' ); ?></strong>
			</label>
		</div>
	...
	<a class="btn" id="add-shortcode"><?php echo __( 'Add Shortcode', 'ulysses' ); ?></a>
		</div>
	</div>
</body>
</html>