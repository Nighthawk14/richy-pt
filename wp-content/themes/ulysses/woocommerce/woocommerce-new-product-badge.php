	...
	if ( ! class_exists( 'WC_nb' ) ) {
		...
		class WC_nb {
			...
			function init() {
				...
				add_action( 'woocommerce_new_item', array( $this, 'woocommerce_show_product_loop_new_badge' ), 30 ); 	// The new badge function
			...
			*/
			...
			function woocommerce_show_product_loop_new_badge() {
				...
				if ( ( time() - ( 60 * 60 * 24 * $newness ) ) < $postdatestamp ) { // If the product was published within the newness time frame display the new badge
					echo '<span class="item-new">' . __( 'New', 'woocommerce-new-badge' ) . '</span>';
				}
			}
		}
